...
import com.smartexpenseai.app.databinding.FragmentCategoriesBinding
import com.smartexpenseai.app.constants.Categories
...
class CategoriesFragment : Fragment() {
        ...
        override fun onReceive(context: Context?, intent: android.content.Intent?) {
            if (intent?.action == "com.expensemanager.MERCHANT_CATEGORY_CHANGED") {
                logger.debug(
                    ...
                    "Received merchant category change broadcast, refreshing categories."
                )
    ...
    private fun updateUI(uiState: CategoriesUIState) {
        logger.debug("updateUI", "Updating UI with ${uiState.categories.size} categories")
    ...
    private fun loadRealCategoryData() {
        lifecycleScope.launch {
            try {
                logger.debug("loadRealCategoryData", "Loading category data from repository...")
                ...
                logger.debug(
                    ...
                    "Categories Date Range: ${startDate} to ${endDate}"
                )
                logger.debug(
                    "loadRealCategoryData",
                    "Categories Raw Transactions Count: ${
                ...
                logger.debug(
                    "loadRealCategoryData",
                    "Repository returned ${categorySpendingResults.size} category results"
                )
                ...
                if (categorySpendingResults.isNotEmpty()) {
                    ...
                    val transactionCategoryData = categorySpendingResults.map { categoryResult ->
                        val lastTransactionText =
                            categoryResult.last_transaction_date?.let { formatLastTransaction(it) }
                                ?: ""

                        logger.debug(
                            "loadRealCategoryData",
                            "Repository Category: ${categoryResult.category_name}, Amount: ₹${
                                String.format(
                                    "%.0f",
                                    categoryResult.total_amount
                                )
                    ...
                    val missingCategories: List<CategoryItem> =
                        allCategories.filter { !existingCategoryNames.contains(it) }
                    ...
                    val categoryData: List<CategoryItem> =
                        (transactionCategoryData + missingCategories)
                    ...
                    logger.debug(
                        "loadRealCategoryData",
                        "Categories Calculated Total: ₹${String.format("%.0f", totalSpent)}"
                    )
                    val categoriesWithPercentages = categoryData.map { categoryItem ->
                        val percentage =
                            if (totalSpent > 0) ((categoryItem.amount / totalSpent) * 100).toInt() else 0
                        categoryItem.copy(
                    ...
                    logger.debug(
                        "loadRealCategoryData",
                        "No data in repository, falling back to SMS reading..."
                    )
                ...
                Toast.makeText(
                    requireContext(),
                    "SMS permission required for real data",
                    Toast.LENGTH_SHORT
                ).show()
            } catch (e: Exception) {
                // Error loading data - show empty state
                showEmptyState()
                Toast.makeText(requireContext(), "Error loading category data", Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
    ...
    private fun loadCategoryDataFallback() {
        lifecycleScope.launch {
            try {
                logger.debug(
                    "loadCategoryDataFallback",
                    "[SMS] Loading category data from SMS as fallback..."
                )
                ...
                if (syncedCount > 0) {
                    ...
                    if (categorySpendingResults.isNotEmpty()) {
                        ...
                        val transactionCategoryData =
                            categorySpendingResults.map { categoryResult ->
                                val lastTransactionText =
                                    categoryResult.last_transaction_date?.let {
                        ...
                        categoriesAdapter.submitList(categoriesWithPercentages)
                        binding.tvTotalSpending.text = "₹${String.format("%.0f", totalSpent)}"
                        binding.tvCategoriesCount.text =
                            "${categoriesWithPercentages.size} Categories"
                    } else {
                        showEmptyState()
                    }
                } else {
                ...
                logger.warn(
                    "loadCategoryDataFallback",
                    "SMS permission denied for fallback loading"
                )
                showEmptyState()
            } catch (e: Exception) {
                logger.error("loadCategoryDataFallback", "Error in fallback loading", e)
                showEmptyState()
            }
        }
    }
    ...
    private fun setupClickListeners() {
        ...
        binding.tabLayoutCategoryFilter.addOnTabSelectedListener(object :
    ...
    private fun showAddCategoryDialog() {
        ...
        dialogView.findViewById<MaterialButton>(R.id.btn_add).setOnClickListener {
            val categoryName =
                dialogView.findViewById<TextInputEditText>(R.id.et_category_name).text.toString()
            val selectedEmoji =
                dialogView.findViewById<TextInputEditText>(R.id.et_emoji).text.toString()
    ...
    private fun showQuickAddDialog() {
        ...
        dialogView.findViewById<MaterialButton>(R.id.btn_add).setOnClickListener {
            ...
            val merchant =
                dialogView.findViewById<TextInputEditText>(R.id.et_merchant).text.toString()
            ...
            if (amount.isNotEmpty() && merchant.isNotEmpty()) {
                try {
                    ...
                    viewModel.handleEvent(
                        CategoriesUIEvent.QuickAddExpense(
                            amountValue,
                            merchant,
                            category
                        )
    ...
    private fun showCategoryActionDialog(
        categoryItem: CategoryItem,
        anchorView: android.view.View
    ) {
        logger.debug(
            "showCategoryActionDialog",
            "Long press detected for category: ${categoryItem.name}"
        )
        ...
        if (Categories.isSystemCategory(categoryItem.name)) {
            logger.debug(
                "showCategoryActionDialog",
                "Category '${categoryItem.name}' is a system category - actions not allowed"
            )
            logger.debug(
                "showCategoryActionDialog",
                "System categories: ${Categories.DEFAULT_CATEGORIES}"
            )
            Toast.makeText(
                requireContext(),
            ...
            logger.debug(
                "showCategoryActionDialog",
                "Category '${categoryItem.name}' is a custom category - allowing actions"
            )
        }
        ...
        try {
            ...
            logger.debug("showCategoryActionDialog", "Custom dialog shown")
        } catch (e: Exception) {
            logger.error("showCategoryActionDialog", "Custom dialog failed", e)
    ...
    private fun showCategoryActionBottomSheet(categoryItem: CategoryItem) {
        ...
        val view = LayoutInflater.from(requireContext())
            .inflate(R.layout.bottom_sheet_category_actions, null)

        // Set up category info
        view.findViewById<android.widget.TextView>(R.id.tv_category_name)?.text = categoryItem.name
        view.findViewById<android.widget.TextView>(R.id.tv_category_emoji)?.text =
            categoryItem.emoji
        ...
        logger.debug(
            "showCategoryActionDialog",
            "BottomSheet dialog shown for category: ${categoryItem.name}"
        )
    }
    ...
    private fun showRenameCategoryDialog(categoryItem: CategoryItem) {
        ...
        dialogView.findViewById<MaterialButton>(R.id.btn_add).setOnClickListener {
            ...
            if (newName.isNotEmpty() && (newName != categoryItem.name || newEmoji != categoryItem.emoji)) {
                logger.debug("showRenameCategoryDialog","Triggering rename from '${categoryItem.name}' to '$newName' with emoji '$newEmoji'")
                viewModel.handleEvent(
                    CategoriesUIEvent.RenameCategory(
                        categoryItem.name,
                        newName,
                        newEmoji
                    )
    ...
    private fun showDeleteCategoryDialog(categoryItem: CategoryItem) {
        logger.debug("showDeleteCategoryDialog",
            "Showing delete dialog for category: ${categoryItem.name}"
        )
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton("Delete") { _, _ ->
                logger.debug(
                    ...
                    "Triggering delete for category: ${categoryItem.name}"
                )
            ...
            .setNegativeButton("Cancel") { dialog, _ ->
                logger.debug("showDeleteCategoryDialog", "Delete dialog cancelled")
                dialog.dismiss()
            }
            .show()
    }
    ...
    private fun showSortOptions() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setItems(options) { _, which ->
                ...
                Toast.makeText(requireContext(), "Sorted by ${options[which]}", Toast.LENGTH_SHORT)
                    .show()
            }
            .show()
    }
